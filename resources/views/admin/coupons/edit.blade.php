                ...
                <form action="{{ route('coupons.update', $coupon->id) }}" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <!-- ✅ Fixed: Proper column structure, no nesting -->
                            <div class="form-group">
                                <label for="expired_at">{{ __('messages.expired_at') }} <span class="text-danger">*</span></label>
                                <input type="date" class="form-control" id="expired_at" name="expired_at" value="{{ old('expired_at', $coupon->expired_at ? date('Y-m-d', strtotime($coupon->expired_at)) : '') }}" required>
                            </div>
                        </div>
                    </div>
...
@section('script')
    <script>
        $(document).ready(function() {
            // Generate random coupon code (only if field is empty)
