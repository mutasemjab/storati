        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title">{{ __('messages.Edit_Shop') }}</h3>
                ...
                <form action="{{ route('shops.update', $shop) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="col-md-8">
                                <div class="card">
                                    <div class="card-header">
                                        <h5>{{ __('messages.Shop_Information') }}</h5>
                                    </div>
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                            ...
                                            <select class="form-control @error('gender') is-invalid @enderror" name="gender" required>
                                                ...
                                                <option value="man" {{ old('gender', $shop->gender) == 'man' ? 'selected' : '' }}>{{ __('messages.Man') }}</option>
                                                <option value="woman" {{ old('gender', $shop->gender) == 'woman' ? 'selected' : '' }}>{{ __('messages.Woman') }}</option>
                                                <option value="both" {{ old('gender', $shop->gender) == 'both' ? 'selected' : '' }}>{{ __('messages.Both') }}</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save"></i> {{ __('messages.Update_Shop') }}
                        </button>
                        <a href="{{ route('shops.index') }}" class="btn btn-secondary">
                            {{ __('messages.Cancel') }}
                        </a>
